...
_rotr.apis.loginIn = function (ctx) {
    ...
    return new Promise(function(resolve, reject){
        Query(`select userId from user where userName="${userName}" and userPwd="${userPwd}"`).then(result=>{
            let userId=null;
            if(result.length){
                userId=result[0].userId
            }
            resolve({isException:true,data:userId});
        }).catch(error=>{
            reject(error)
        });
    });
...
_rotr.apis.updateUserInfo = function (ctx) {
    ...
    return new Promise(function(resolve, reject){
        getQueryDatas(`update user set userName='${userName}',realName='${realName}',description='${description}',sign='${sign}' where userId=${userId}`,resolve,reject);
    });
};
/**
 * 用户头像上传*/
...
_rotr.apis.checkMail = function (ctx) {
    ...
    return new Promise(function(resolve, reject){
        getQueryDatas(`select userName from user where userId=${userId} and userEmail='${email}'`,resolve,reject);
    });
};
...
_rotr.apis.updatePwd = function (ctx) {
    ...
    return new Promise(function(resolve, reject){
        getQueryDatas(`update  user set userPwd='${someWords}' where userId=${userId}`,resolve,reject);
    });
};
...
_rotr.apis.checkMailMsg = function (ctx) {
    ...
    return new Promise(( resolve, reject ) => {
        _sendMail.sendMail({
            ...
            subject: '您正在进行邮箱验证',
            html: '<h1>您的邮箱验证码为：'+checkWords+'，此验证码10分钟内有效</h1>'
        }, ( err, rows) => {
            if ( err ) {
                reject( {isException:false,data:err})
            } else {
                resolve({isException:true,data:null})
            }
        })
    });
};
