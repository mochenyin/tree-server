...
export default class GameGobang extends React.Component<Props,State> {
    ...
    clickBarder:React.MouseEventHandler<HTMLDivElement>=e=> {
        ...
        let X=this.getPosition(e.clientX-this.state.initLeft-this.props.initLeft);
        let Y=this.getPosition(e.clientY-this.state.initTop);
        console.error(this.state.initLeft,this.state.initTop);
        if(X===-1||Y===-1) return;
    ...
    render() {
        ...
        return ([
            <div key='barder' className="barder" ref={this.barderRef} onClick={this.clickBarder}>
            ...
            <span key='title' className='gameTitle'>黑子数：{datas.filter(item=>item.flb===0).length}&nbsp;&nbsp;&nbsp;
